<x-layout :title="$PageTitle">
  <form action="{{ route('posts.update', $post) }}" method="POST">
          ...
          <div class="col-span-6">
            ...
            <div class="mt-2">
              <textarea id="content" name="content" rows="3"
                class="{{ $errors->has('content') ? 'outline-red-500' : 'outline-gray-300' }} block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1  placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">{{ old('content', $post->content) }}</textarea>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="mt-6 flex items-center justify-end gap-x-6">
      <a href="{{ route('posts.show', $post->id) }}" class="text-sm/6 block font-semibold text-gray-900">Cancel</a>
      <button type="submit"
